...
{
    if (scrollView == self.tableView && _didScroll) {
        _didScroll();
    }
}
...
- (void)loadLargeImage:(UITapGestureRecognizer *)recognizer {
    ...
    [thumbURL replaceCharactersInRange:NSMakeRange(thumbURL.length - 10, 10) withString:[thumbURL substringWithRange:NSMakeRange(thumbURL.length - 4, 4)]];
